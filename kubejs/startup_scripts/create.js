     ...
     */
      function myst(input,output){
        ...
        for(let i = 0;i< input.lenght()-1;i++){
          $MysteriousItemConversionCategory.RECIPES.add($ConversionRecipe.create(input[i],input[i+1]))
        }
      }
      }
      ...
      myst(['kubejs:illuminati', 'tiab:time_in_a_bottle'])
      myst(['kubejs:soul', 'tombstone:soul_receptacle'])
      myst(['minecraft:cauldron', 'reactive:crucible'])
      myst(['thoriumreactors:molten_salt_bucket', 'thoriumreactors:heated_molten_salt_bucket', 'thoriumreactors:depleted_molten_salt_bucket'])
      myst(['thoriumreactors:heated_molten_salt_bucket', 'thoriumreactors:steam_bucket'])
      myst(['exoticbirds:nest', 'exoticbirds:mystery_egg', 'exoticbirds:phoenix_egg'])
